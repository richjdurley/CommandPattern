...
import rd.command.framework.api.exceptions.ErrorResponseTransformer;
import rd.command.json.JSONUtil;
import reactor.core.publisher.Mono;

import java.util.logging.Level;

import static rd.command.framework.api.filters.SharedHTTPHeaders.addHeaders;
...
public class GlobalTransactionFilter implements WebFilter {
    ...
    private Mono<Void> doManualErrorProcessing(ServerWebExchange serverWebExchange, WebFilterChain webFilterChain, Throwable e) {
        ...
        SharedHTTPHeaders.addHeaders(serverWebExchange, MediaType.APPLICATION_JSON).getResponse().setStatusCode(HttpStatus.valueOf(ErrorResponseTransformer.transformToErrorResponse(e).getStatusCode()));
